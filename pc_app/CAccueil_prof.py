...
class AppBancMotProf(QWidget):
    def __init__(self, identifiant,role,moteur_choisi=None):
        ...
        self.ui.pb_choix_fichierCSV.clicked.connect(self.ajouterEleveCsv)
        ...
        self.ui.pb_supprimer_utilisateur.clicked.connect(self.supprimerUtilisateur)
        self.ui.pb_modifier_utilisateur.clicked.connect(self.modifierUtilisateur)
        self.ui.pb_valider_capteur.clicked.connect(self.validerCapteur)
        self.ui.pb_reinit_mdp.clicked.connect(self.resetMdp)
        self.ui.pb_desactiver_2AF.clicked.connect(self.desactiver2AF)
    ...
    def validerAxe(self):
        ...
        host, user, db_password, database = 'localhost', 'prof', 'CestGenialCeBts2025', 'eguidat_banc_moteur'  # Paramètres de connexion à la base de données
        try:
    ...
    def arreterCycle(self):
        ...
        try:
            ...
            # Sécurisation par défaut des listes
            def safe_list(liste):
                return [str(x) if x is not None else "0" for x in liste]

            tension = "[" + ";".join(safe_list(self.tension)) + "]"
            courant = "[" + ";".join(safe_list(self.courant)) + "]"
            couple = "[" + ";".join(safe_list(self.couple)) + "]"
            vitesse = "[" + ";".join(safe_list(self.vitesse)) + "]"
            puissance = "[" + ";".join(safe_list(self.puissance)) + "]"
            temps = "[" + ";".join(safe_list(self.temps)) + "]"
            moteur_choisi = self.moteur_choisi

            requete = """INSERT INTO essai (id_essai, date_essai, identifiant_eleve_essai,
                        tension, courant, couple, vitesse_moteur, id_moteur, puissance, temps)
    ...
    def ajouterEleve(self):
        nom = self.ui.lE_nom_eleve.text().strip()  # Récupérer et nettoyer le nom
        prenom = self.ui.lE_prenom_eleve.text().strip()  # Récupérer et nettoyer le prénom
        classe = self.ui.lE_classe_eleve.text()  # Récupérer la classe
        identifiant = prenom[0].lower() + nom.lower()  # Créer l'identifiant (initiale prénom + nom)
        mdp = "changeme"  # Mot de passe par défaut
        mdp_hash = bcrypt.hashpw(mdp.encode(), bcrypt.gensalt())  # Hash du mot de passe
        host = 'localhost'
        user = 'prof'
        db_password = 'CestGenialCeBts2025'
        database = 'eguidat_banc_moteur'
        ...
        try:
            ...
            if resultat_identifiant is None:  # Si l'identifiant n'existe pas déjà
                ...
                cursor.fetchall()  # Vider le buffer

                if resultat_classe is None:  # Si la classe n'existe pas, l'ajouter
                    ...
                    id_classe = cursor.fetchone()['id_classe']
                else:
                    id_classe = resultat_classe['id_classe']  # Sinon récupérer l'ID existant

                # Ajouter l'élève dans la table utilisateur
                requete_ajout_eleve = "INSERT INTO utilisateur (identifiant, mdp, role, classe_id, nom, prenom) VALUES (%s, %s, 'eleve', %s, %s, %s);"
                cursor.execute(requete_ajout_eleve, (identifiant, mdp_hash, id_classe, nom, prenom))
                connection.commit()
            ...
            self.recuperer_utilisateur()
            self.ui.lE_nom_eleve.clear()
            self.ui.lE_prenom_eleve.clear()
            self.ui.lE_classe_eleve.clear()

        except mysql.connector.Error as e:
            print(f"Erreur MySQL : {e}")

    def ajouterEleveCsv(self):
        # Ouvre une boîte de dialogue pour choisir le fichier CSV
        fichier_csv, _ = QFileDialog.getOpenFileName(self, "Choisir un fichier CSV", "", "Fichiers CSV (*.csv)")
        if not fichier_csv:
            self.ui.l_nomFichier.setText("Aucun fichier sélectionné.")
            return
        self.ui.l_nomFichier.setText(f"Fichier choisi : {fichier_csv}")

        # Connexion à la base de données
        connection = mysql.connector.connect(host='localhost', user='prof', password='CestGenialCeBts2025', database='eguidat_banc_moteur')
        cursor = connection.cursor(dictionary=True)

        # Ouvre le fichier CSV avec le bon séparateur
        with open(fichier_csv, newline='', encoding='utf-8') as fichier:
            ...
            for row in reader:
                ...
                cursor.execute("SELECT identifiant FROM utilisateur WHERE identifiant=%s;", (identifiant,))
                resultat_identifiant = cursor.fetchone()
                if resultat_identifiant is None:
                    # Vérifie si la classe existe déjà
                    cursor.execute("SELECT nom_classe, id_classe FROM classe WHERE nom_classe=%s;", (classe,))
                    resultat_classe = cursor.fetchone()
                    if resultat_classe is None:
                        # Insère la classe si elle n'existe pas
                        cursor.execute("INSERT INTO classe(nom_classe) VALUES(%s);", (classe,))
                        connection.commit()
                        # Récupère l'id de la classe insérée
                        cursor.execute("SELECT id_classe FROM classe WHERE nom_classe=%s;", (classe,))
                        id_classe = cursor.fetchone()['id_classe']
                    else:
                        id_classe = resultat_classe['id_classe']

                    # Insère l'élève dans la table utilisateur
                    cursor.execute(
                        "INSERT INTO utilisateur (identifiant, mdp, role, classe_id, nom, prenom) VALUES (%s, %s, 'eleve', %s, %s, %s);",
                        (identifiant, mdp_hash, id_classe, nom, prenom)
                    )
                    connection.commit()
                else:
                    # Optionnel: gérer les doublons d'identifiant (ici on ignore)
                    print(f"Identifiant {identifiant} déjà existant, élève non ajouté.")

        cursor.close()
        connection.close()
        self.recuperer_utilisateur()
        self.ui.l_nomFichier.setText("Import CSV terminé.")
    ...
    def recuperer_utilisateur(self):
        self.ui.tE_listeUtilisateur.resizeRowsToContents()
        host = 'localhost'
        user = 'prof'
        db_password = 'CestGenialCeBts2025'
        database = 'eguidat_banc_moteur'
    ...
    def modifierUtilisateur(self):
        try:
            ...
            cursor = connection.cursor()
            row = self.ui.tE_listeUtilisateur.currentRow()
            if row == -1:
                self.ui.l_infos_utilisateur.setText("Veuillez sélectionner un moteur.")
                return

            identifiant = self.ui.tE_listeUtilisateur.item(row, 0).text()
            role = self.ui.tE_listeUtilisateur.item(row, 1).text()
            classe = self.ui.tE_listeUtilisateur.item(row, 2).text()
            nom = self.ui.tE_listeUtilisateur.item(row, 3).text()
            prenom = self.ui.tE_listeUtilisateur.item(row, 4).text()
            requete_classe="""SELECT id_classe FROM classe WHERE nom_classe=%s"""
            cursor.execute(requete_classe,(classe))
            requete = """UPDATE utilisateur
                        SET role=%s, classe_id=%s, nom=%s, prenom=%s
                        WHERE identifiant=%s"""
            cursor.execute(requete, (role,classe,nom,prenom,identifiant))
            connection.commit()
            connection.close()

            self.ui.l_infos_utilisateur.setText("Moteur modifié avec succès")
        except Exception as e:
            print(e)
            self.ui.l_infos_utilisateur.setText("Erreur lors de la modification")
    ...
    def supprimerUtilisateur(self):
        try:
            ...
            row = self.ui.tE_listeUtilisateur.currentRow()
            if row == -1:
                self.ui.l_infos_utilisateur.setText("Veuillez sélectionner un utilisateur à supprimer.")
                return
            id_eleve = self.ui.tE_listeUtilisateur.item(row, 0).text()
            requete_essais = "DELETE FROM essai WHERE identifiant_eleve_essai = %s"
            cursor.execute(requete_essais, (id_eleve,))
            requete = """DELETE FROM utilisateur WHERE identifiant=%s"""
            cursor.execute(requete, (id_eleve,))
            connection.commit()
            self.ui.tE_listeUtilisateur.removeRow(row)
            connection.close()
            self.ui.l_infos_utilisateur.setText("Utilisateur supprimé avec succès")
        except Exception as e:
            print(e)
            self.ui.l_infos_utilisateur.setText("Erreur lors de la suppression")
    ...
    def resetMdp(self):
        try:
            ...
            row = self.ui.tE_listeUtilisateur.currentRow()
            if row == -1:
                self.ui.l_infos_utilisateur.setText("Veuillez sélectionner un utilisateur pour réinitialiser son mot de passe.")
                return
            id_eleve = self.ui.tE_listeUtilisateur.item(row, 0).text()
            mdp = "changeme"  # Mot de passe par défaut
            mdp_hash = bcrypt.hashpw(mdp.encode(), bcrypt.gensalt())  # Hash du mot de passe
            requete = """UPDATE utilisateur
                        SET mdp=%s, premiere_connexion=""
                        WHERE identifiant=%s"""
            cursor.execute(requete, (mdp_hash,id_eleve))
            connection.commit()
            self.ui.tE_listeUtilisateur.removeRow(row)
            connection.close()
            self.ui.l_infos_utilisateur.setText("Mot de passe réinitialisé avec succès")
        except Exception as e:
            print(e)
            self.ui.l_infos_utilisateur.setText("Erreur lors de la suppression")
        self.recuperer_utilisateur()
        
    def desactiver2AF(self):
        try:
            connection = mysql.connector.connect(
                host='localhost',
                user='prof',
            ...
            row = self.ui.tE_listeUtilisateur.currentRow()
            if row == -1:
                self.ui.l_infos_utilisateur.setText("Veuillez sélectionner un utilisateur pour désactiver sa double authentification.")
                return
            id_eleve = self.ui.tE_listeUtilisateur.item(row, 0).text()
            requete = """UPDATE utilisateur
                        SET secret=""
            ...
            self.ui.tE_listeUtilisateur.removeRow(row)
            connection.close()
            self.ui.l_infos_utilisateur.setText("Double authentification désactivé avec succès")
        except Exception as e:
            print(e)
            self.ui.l_infos_utilisateur.setText("Erreur lors de la désactivation")
        self.recuperer_utilisateur()
